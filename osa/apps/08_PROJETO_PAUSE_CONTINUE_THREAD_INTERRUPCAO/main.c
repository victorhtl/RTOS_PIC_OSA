...
void INTERRUPT_LOW() iv 0x0018 ics ICS_AUTO {
  ...
  OS_EnterInt();
   ...
   if(INTCON.TMR0IF == 1)
   {
      ...
      //PORTD.RD6 = ~LATD.RD6; //Inverte o estado do LED

        if (OS_Msg_Check_I (msg_pause)) {     // If message exists
           OS_Msg_Accept_I(msg_pause, msg);  // then accept it
           
           LATD.RD6 = ~LATD.RD6; //Inverte o estado do LED
       }

        if(!OS_Msg_Check_I (msg_int)){
           OS_Msg_Send_I(msg_int, "int");
        }
...
void main (void)
{
    ...
    OS_Task_Create(3, Task_LCD);      //Cria task LCD (máxima prioridade)
    OS_Task_Create(4, Task_BUTTON0);      //Cria task LE2 (máxima prioridade)
...
void Task_LED0 (void)
{
   OST_MSG msg;
    for(;;)
    {
      ...
      if (OS_Msg_Check (msg_int)) {     // If message exists
           OS_Msg_Accept(msg_int, msg);  // then accept it
            LATD.RD0 = ~LATD.RD0;
       }
        OS_Yield();
    }
